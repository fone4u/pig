...
SONG_online_tj_son = filter SONG_online_tj_join by (pid=='online_280' or pid=='online_281');
SONG_online_tj_son_group = group SONG_online_tj_son by (pid,id,name);
SONG_online_tj_son_count = foreach SONG_online_tj_son_group{
    uv = distinct SONG_online_tj_son.uid;
    generate flatten(group),COUNT(SONG_online_tj_son) as pv,COUNT(uv) as uv,'$day';
};
STORE SONG_online_tj_son_count INTO 'song_listeninfo_son' USING  org.apache.pig.piggybank.storage.DBStorage('com.mysql.jdbc.Driver','jdbc:mysql://10.0.2.100:3306/pig','user','stat^2012','insert into song_listeninfo_son(pid,sonid,sname,pv,uv,stat_date) values(?,?,?,?,?,?)');
