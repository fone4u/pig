...
GETKEY = foreach AA generate ip,data#'module' as module,data#'type' as type,data#'origin' as origin,(param#'hid' is null?param#'uid':CONCAT(param#'uid',param#'hid')) as uid;
SEARCH_listen_menu_log = filter GETKEY by (module=='song' and ((type=='listen_info' and origin matches 'search.*') or (type=='show-menu')) );
SEARCH_listen_menu = foreach SEARCH_listen_menu_log generate origin,uid;
