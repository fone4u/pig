...
--xia zai wan cheng
DOWNLOAD_app_adid = filter GETKEY by module=='download' and type=='app' and origin matches 'game.*' and not origin matches 'game-tucao.*';
DOWNLOAD_app_adid_adname1 = foreach DOWNLOAD_app_adid{
...
DOWNLOAD_app_adid_adname = filter DOWNLOAD_app_adid_adname1 by key1 is not null;
DOWNLOAD_app_adid_adname_group = group DOWNLOAD_app_adid_adname by  (key1,key2,key3,key4,key5);
DWONLOAD_app_adid_adname_count = foreach DOWNLOAD_app_adid_adname_group{
    uv = distinct DOWNLOAD_app_adid_adname.uid;
    generate flatten(group),COUNT(DOWNLOAD_app_adid_adname),COUNT(uv),'$day';
};
STORE DWONLOAD_app_adid_adname_count into 'recommend_download_end' USING  org.apache.pig.piggybank.storage.DBStorage('com.mysql.jdbc.Driver','jdbc:mysql://10.0.2.100:3306/pig','user','stat^2012','insert into recommend_download_end(channel,adid,order_num,adname,ds,pv,uv,stat_date) values(?,?,?,?,?,?,?,?)');
