	...
	public String exec(Tuple input) throws IOException {
		try {
			if (input == null || input.size() != 2) {
				...
				warn("this method must have two args",
			...
			for (int i = 0; i < surl.length; i++) {
				...
				if (kv[0].equals(keyWord)) {// 此处出现可异常，kv[0] 空 所以放在try里面，异常数据可能是像( =&= )
					return kv[1];
				}
